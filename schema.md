...
| `hypothese_clinique` | traversée — **vigilance plume maximale** (§5) | — | prose : **ce qui se joue** — la question qui interroge l'essence de la prise en charge et l'oriente. **Ni le récit** (ce qui se dégage : `recit`), **ni l'accompagnement** (où on va : `accompagnement`) : ce sont désormais des objets distincts. Jamais un état d'équilibre atteint — une lecture en mouvement | la **spirale** = la chaîne de révisions (`ref_id` = hypothèse révisée) ; se lève par `levee` en collège ; jamais contrat opposable |
