...
-- 3. **Sans gestation.** La machine voit **qu'il** existe une gestation ; jamais ce qui
--    gestationne (§4, catalogue). Le masque est dans la vue, pas dans une consigne.
--
...
-- ── Domaine 2 — tout le flux, rien de masqué, sauf ce qui gestationne ─────────
-- « En dépôts, la projection ne masque rien : chaque pli s'affiche. » Aucun filtre de
-- correction ici : il n'y a que des états datés du regard.
...
WITH flux AS (
  SELECT d.id, d.ipp, d.depose_le, d.auteur_id, d.cadre, d.nature,
         ...
         CASE WHEN d.nature = 'gestation' THEN NULL ELSE d.contenu END AS contenu
  FROM depot.depots d
)
SELECT * FROM flux;
COMMENT ON VIEW lecture.depots IS
  'La machine voit QU''une gestation existe, jamais ce qui gestationne. Le masque est ici, dans la fenêtre — pas dans un prompt.';
